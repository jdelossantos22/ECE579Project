...
# PREDICATE - ON, ONSHELFSTAND, TOPBOTTLE, HOLDING, ARMEMPTY
class PREDICATE:
    def __str__(self):
        pass
...
class ONSHELFSTAND(PREDICATE): #onshelfstand is technically exactly the same as ontable
    ...
    def __str__(self):
        return "ONSHELFSTAND({X})".format(X=self.X)
...
class TOPBOTTLE(PREDICATE): #clear
    ...
    def __str__(self):
        return "TOPBOTTLE({X})".format(X=self.X)
        self.X = X
...
class StackOp(Operation):
    ...
    def precondition(self):
        return [TOPBOTTLE(self.Y), HOLDING(self.X)]
...
class UnstackOp(Operation):
    ...
    def precondition(self):
        return [ARMEMPTY(), ON(self.X, self.Y), TOPBOTTLE(self.X)]
...
if __name__ == '__main__':
    ...
    alreadyinshelf = initial_state[-3]
    VeryTop = str(alreadyinshelf)[-2]
    newinshelf= initial_state[-2]
    BottomBottel = str(newinshelf)[-2]
    middle_1 = initial_state[-4]
    e = str(middle_1)[-2]
    w = str(middle_1)[-4]
    middle_2 = initial_state[-5]
    z = str(middle_2)[-2]
    v = str(middle_2)[-4]
